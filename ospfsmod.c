...
{
	uint32_t i, inodeNumber = 0;
	void* inodeArray = ospfs_block(ospfs_super->os_firstinob);
	for(i=OSPFS_INODESIZE;i<(ospfs_super->os_ninodes*OSPFS_INODESIZE);i = i+OSPFS_INODESIZE) //Skip the 1st inode as it's to be never used
	{
		inodeNumber++;
		if(((ospfs_inode_t*)(inodeArray+i))->oi_nlink == 0)
			return inodeNumber;
	}
	return 0;
}
...
{
	uint32_t * ptr = ospfs_block(blockNumber);
	int i;
...
ospfs_dir_readdir(struct file *filp, void *dirent, filldir_t filldir)
{
	...
	while (r == 0 && ok_so_far >= 0 && f_pos >= 2) {
		...
		directoryNumber = (f_pos-2)*OSPFS_DIRENTRY_SIZE;
		od = ospfs_inode_data(dir_oi, directoryNumber);
...
allocate_block(void)
{
	...
	uint32_t index = 0;
	uint32_t numBlocks = ospfs_super->os_nblocks;
	void* bitmapBlock = ospfs_block(OSPFS_FREEMAP_BLK); //Pointer to bitmap block

	for(index = 0; index < numBlocks; index++) {
		if(bitvector_test(bitmapBlock, index)) {
			bitvector_clear(bitmapBlock,index);
			return index;
		}
	}
	return 0;
...
free_block(uint32_t blockno)
{
	...
	if(blockno == 0 || blockno == 1 || blockno == 2)
		return;
	if(bitvector_test(bitmapBlock, blockno))
		return;
	else
		bitvector_set(bitmapBlock, blockno);
	return;
}
...
// current number of blocks in file
	...
	uint32_t * secondIndirectBlock;
	void* bitmapBlock = ospfs_block(OSPFS_FREEMAP_BLK); 
	int index, tempBlockNumber, index2, indexOfFirstIndirect;
	...
	if(n == OSPFS_NDIRECT)
	{
		...
		if(allocated[0])
		{
			...
			clearBlock(allocated[0]);
			oi->oi_indirect = allocated[0];
			allocated[1] = allocate_block();
			if(allocated[1])
			{
				newBlock[0]=allocated[1];
				clearBlock(newBlock[0]);
			}
			else
			{
	...
	if(n > OSPFS_NDIRECT && n < OSPFS_NDIRECT + OSPFS_NINDIRECT)
	{
		if(bitvector_test(bitmapBlock,oi->oi_indirect))
			return -EIO;
		index = n - OSPFS_NDIRECT;
		allocated[0] = allocate_block();
		if(allocated[0])
		{
			uint32_t * indirectBlock = ospfs_block(oi->oi_indirect);
			indirectBlock[index]=allocated[0];
		}
		else
			return -ENOSPC;
	}
	...
	if(n == OSPFS_NDIRECT + OSPFS_NINDIRECT)
	{
		...
		if(allocated[0])
		{
			...
			{
				newBlock[0]=allocated[1];
				clearBlock(allocated[1]);
				tempBlockNumber = allocate_block();
				if(tempBlockNumber)
				{
					newBlock2 = ospfs_block(allocated[1]);
					newBlock2[0] = tempBlockNumber;
					clearBlock(tempBlockNumber);
				}
				else
				{
					free_block(allocated[0]);
	...
	if(n > OSPFS_NDIRECT + OSPFS_NINDIRECT && n < OSPFS_NDIRECT + OSPFS_NINDIRECT + OSPFS_NINDIRECT*OSPFS_NINDIRECT)
	{
		if(bitvector_test(bitmapBlock,oi->oi_indirect2))
			return -EIO;
		index2 = n - OSPFS_NDIRECT - OSPFS_NINDIRECT;
		indexOfFirstIndirect = index2/OSPFS_NINDIRECT;
		
		if(index2%OSPFS_NINDIRECT == 0)
		{
			allocated[0]=allocate_block();
			if(allocated[0])
			{
				firstIndirectBlock = ospfs_block(oi->oi_indirect2);
				firstIndirectBlock[indexOfFirstIndirect]= allocated[0];
				clearBlock(allocated[0]);
			}
			else
				return -ENOSPC;
		...
		{
			firstIndirectBlock = ospfs_block(oi->oi_indirect2);
			if(bitvector_test(bitmapBlock,firstIndirectBlock[indexOfFirstIndirect]))
				return -EIO;
			secondIndirectBlock = ospfs_block(firstIndirectBlock[indexOfFirstIndirect]);
			secondIndirectBlock[index2%OSPFS_NINDIRECT] = allocated[1];
			clearBlock(allocated[1]);
		}
		else
		{
...
// current number of blocks in file
	uint32_t n = ospfs_size2nblocks(oi->oi_size);
	uint32_t indexOfFirstIndirect;
	uint32_t indexOfSecondIndirect;
	uint32_t * indirectPointer1;
	uint32_t * indirectPointer2;
	void* bitmapBlock = ospfs_block(OSPFS_FREEMAP_BLK); //Pointer to bitmap block
	...
	if(n<=OSPFS_NDIRECT)
	{
		if(bitvector_test(bitmapBlock,oi->oi_direct[n-1]))
			return -EIO;
	...
	if(n == OSPFS_NDIRECT+1)
	{
		if(bitvector_test(bitmapBlock,oi->oi_indirect))
			return -EIO;
		indirectPointer1 = ospfs_block(oi->oi_indirect);
		if(bitvector_test(bitmapBlock,indirectPointer1[0]))
			return -EIO;
		free_block(indirectPointer1[0]);
		free_block(oi->oi_indirect);
		oi->oi_indirect = 0;
	}
	
	if(n > OSPFS_NDIRECT+1 && n <= OSPFS_NDIRECT + OSPFS_NINDIRECT)
	{
		indexOfFirstIndirect = n - OSPFS_NDIRECT -1;
		if(bitvector_test(bitmapBlock,oi->oi_indirect))
			return -EIO;
		indirectPointer1 = ospfs_block(oi->oi_indirect);
		if(bitvector_test(bitmapBlock,indirectPointer1[indexOfFirstIndirect]))
			return -EIO;
		free_block(indirectPointer1[indexOfFirstIndirect]);
		indirectPointer1[indexOfFirstIndirect] = 0;
	}
	...
	if(n == OSPFS_NDIRECT + OSPFS_NINDIRECT+1)
	{
		if(bitvector_test(bitmapBlock,oi->oi_indirect2))
			return -EIO;
		indirectPointer1 = ospfs_block(oi->oi_indirect2);
		if(bitvector_test(bitmapBlock,indirectPointer1[0]))
			return -EIO;
		indirectPointer2 = ospfs_block(indirectPointer1[0]);
		if(bitvector_test(bitmapBlock,indirectPointer2[0]))
			return -EIO;
		free_block(indirectPointer2[0]);
		free_block(indirectPointer1[1]);
		free_block(oi->oi_indirect2);
		oi->oi_indirect2 = 0;
	}
	
	if(n > OSPFS_NDIRECT + OSPFS_NINDIRECT +1 && n <= OSPFS_NDIRECT + OSPFS_NINDIRECT + OSPFS_NINDIRECT * OSPFS_NINDIRECT)
	{	//255
		int index = (n - OSPFS_NDIRECT + OSPFS_NINDIRECT -1);
		indexOfFirstIndirect = (index)/OSPFS_BLKSIZE;
		indexOfSecondIndirect = index%OSPFS_BLKSIZE;
		if(bitvector_test(bitmapBlock,oi->oi_indirect2))
			return -EIO;
		indirectPointer1 = ospfs_block(oi->oi_indirect2);
		if(bitvector_test(bitmapBlock,indirectPointer1[indexOfFirstIndirect]))
			return -EIO;
		indirectPointer2 = ospfs_block(indirectPointer1[indexOfFirstIndirect]);
		if(bitvector_test(bitmapBlock,indirectPointer2[indexOfSecondIndirect]))
			return -EIO;
		free_block(indirectPointer2[indexOfSecondIndirect]);
		indirectPointer2[indexOfSecondIndirect] = 0;
		if(indexOfSecondIndirect == 0)
		{
			free_block(indirectPointer1[indexOfFirstIndirect]);
			indirectPointer1[indexOfFirstIndirect] = 0;
		}
...
change_size(ospfs_inode_t *oi, uint32_t new_size)
{
	...
	while (ospfs_size2nblocks(oi->oi_size) < ospfs_size2nblocks(new_size)) {
		...
		if(r == -ENOSPC)
		{
			new_size = old_size; // Remove blocks
		}
		else if(r == -EIO)
			return r;
	}
...
ospfs_read(struct file *filp, char __user *buffer, size_t count, loff_t *f_pos)
{
	...
	while (amount < count && retval >= 0) {
		...
		if(copy_to_user((void *) buffer, data, n)){
			retval = -EFAULT;
			goto done;
		}
		buffer += n;
...
ospfs_write(struct file *filp, const char __user *buffer, size_t count, loff_t *f_pos)
{
	...
	if((filp->f_flags & O_APPEND))
		*f_pos = oi->oi_size; //Appending, so we set this pointer to be at the end of the file.
	
	// If the user is writing past the end of the file, change the file's
	// size to accomodate the request.  (Use change_size().)
	/* EXERCISE: Your code here */
	if(count > oi->oi_size - *f_pos) {
		int status = change_size(oi, count + *f_pos);
		if(status == -ENOSPC)
			return -ENOSPC;
		else if (status == -EIO)
			return -EIO;
	...
	while (amount < count && retval >= 0) {
		uint32_t blockno = ospfs_inode_blockno(oi, *f_pos);
		uint32_t n;
		uint32_t spaceLeftInBlk;
		char *data;
		
		if (*f_pos == oi->oi_size) { //Cannot write anymore
		...
		/* EXERCISE: Your code here */
		spaceLeftInBlk = OSPFS_BLKSIZE - (*f_pos%OSPFS_BLKSIZE);
		
		if((count-amount) <= spaceLeftInBlk)
			n = count-amount;
		else
			n = spaceLeftInBlk;		
		
		if(copy_from_user(data + (*f_pos%OSPFS_BLKSIZE), (void*)buffer, n)) {
			retval = EFAULT;
			goto done;
		}
		if ((*f_pos + n) >= oi->oi_size) //Update the file size
...
create_blank_direntry(ospfs_inode_t *dir_oi)
{
	...
	/* EXERCISE: Your code here. */
	int index, retval;
	ospfs_direntry_t *returnObject;
	ospfs_direntry_t *od;
	
	for(index = 0; index < dir_oi->oi_size; index += OSPFS_DIRENTRY_SIZE) {
		od = ospfs_inode_data(dir_oi, index);
		if(od->od_ino == 0)
			return od;
	}
	...
	returnObject = ospfs_inode_data(dir_oi, dir_oi->oi_size - OSPFS_DIRENTRY_SIZE);
	//returnObject->od_ino = find_free_inode();
...
ospfs_link(struct dentry *src_dentry, struct inode *dir, struct dentry *dst_dentry) {
	...
	dentryPtr = create_blank_direntry(dir_oi);
	if (IS_ERR(dentryPtr))
		return PTR_ERR(dentryPtr);
	

	strncpy(dentryPtr->od_name, dst_dentry->d_name.name, dst_dentry->d_name.len);	//Copy name to memory
	dentryPtr->od_name[dst_dentry->d_name.len] = '\0'; //Add null byte at the end of string]
  
	src_inode = ospfs_inode(src_dentry->d_inode->i_ino);
	src_inode->oi_nlink++;	//Increment the nlink

	dentryPtr->od_ino = src_dentry->d_inode->i_ino;	// Make memory point to the inode number
...
ospfs_create(struct inode *dir, struct dentry *dentry, int mode, struct nameidata *nd)
{
	...
	if(dentry->d_name.len > OSPFS_MAXNAMELEN)
		return -ENAMETOOLONG;
	entry_ino = find_free_inode(); //Get a free inode
	if(entry_ino == 0) //No more free inode = no more space for new files
		return -ENOSPC;
	blank_direntry = create_blank_direntry(dir_oi);
...
ospfs_symlink(struct inode *dir, struct dentry *dentry, const char *symname)
{
	...
	newInode->oi_nlink = 1;
	memcpy(newInode->oi_symlink, symname, strlen(symname) * sizeof(char));
	newInode->oi_symlink[strlen(symname)] = '\0';
...
ospfs_follow_link(struct dentry *dentry, struct nameidata *nd)
{
	...
	char  *questionMark, *colon, *tmp;
	int questionMarkIndex = 0, colonIndex = 0, endIndex=0;
	
	// Exercise: Your code here
	questionMark = strchr(oi->oi_symlink, '?'); //returns a ptr to the location of '?' or NULL if doesn't exist
	if(questionMark != NULL) {//We may have a conditional symlink
		questionMarkIndex = questionMark - oi->oi_symlink + 1;
		colon = questionMark+1;
		colonIndex = questionMarkIndex+1;
		while(*colon != ':'){
			colon++;
			colonIndex++;
		}
		...
		memmove(colon+1, colon, endIndex+1);
		questionMark[colonIndex-questionMarkIndex] = '\0';

		if(current->uid == 0) //isRoot
			nd_set_link(nd, questionMark+1);
		else 
			nd_set_link(nd, colon+2);
		return (void *) 0;
	}
	//We have no conditional symlink
...
MODULE_AUTHOR("Daniel Yao 004-182-464");
MODULE_AUTHOR("Setiawan Makmur 704-209-040");
